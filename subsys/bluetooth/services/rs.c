...
#ifdef CONFIG_USE_DEFAULT_STS_KEY_IV
// uint32_t sts_key[STS_U32_LEN] = { 0x14EB220F, 0xF86050A8, 0xD1D336AA, 0x14148674 };
// uint32_t sts_iv[STS_U32_LEN] = { 0x1F9A3DE4, 0xD37EC3CA, 0xC44FA8FB, 0x362EEB34 };
#else
// #include "key_override.h" //Placeholder. 
#endif
// union {
// 	uint32_t u32[4];
// 	uint8_t u8[16];
			    ...
			    uint16_t len, uint16_t offset)
{
	...
	return bt_gatt_attr_read(conn, attr, buf, len, offset, &sts_iv[3], sizeof(uint32_t));
}
                        ...
                        uint16_t len, uint16_t offset)
{
	...
	return bt_gatt_attr_read(conn, attr, buf, len, offset, sts_key, sizeof(sts_key));
}
                         ...
                         uint16_t len, uint16_t offset, uint8_t flags)
{
	...
	memcpy(sts_key, (uint8_t *)buf, sizeof(sts_key));
                       ...
                       uint16_t len, uint16_t offset)
{
	...
	return bt_gatt_attr_read(conn, attr, buf, len, offset, sts_iv, sizeof(sts_iv));
}
                        ...
                        uint16_t len, uint16_t offset, uint8_t flags)
{
	...
	memcpy(sts_iv, (uint8_t *)buf, sizeof(sts_iv));
                       ...
                       uint16_t len, uint16_t offset)
{
	...
	return bt_gatt_attr_read(conn, attr, buf, len, offset, timestamp, sizeof(timestamp));
}
...
void bt_rs_set_timestamp(uint64_t *ts)
{
	...
	memcpy(timestamp, ts, sizeof(timestamp));
}
...
int bt_rs_notify_timestamp(struct bt_conn *conn)
{
	...
	int err = bt_gatt_notify(conn, timestamp_attr, timestamp, sizeof(timestamp));
	if (err) {
		LOG_ERR("Failed to send indication: %s", bt_gatt_err_to_str(err));
	}
	return err;
...
static int bt_rs_init()
{
	...
	// /* debug */
	// printk("parsed IV: 0x%08X 0x%08X 0x%08X 0x%08X\n", sts_iv.u32[0], sts_iv.u32[1], sts_iv.u32[2], sts_iv.u32[3]);
	// printk("parsed KEY: 0x%08X 0x%08X 0x%08X 0x%08X\n", sts_key.u32[0], sts_key.u32[1], sts_key.u32[2], sts_key.u32[3]);
